...
router.post('/', async (req, res, next) => {
	...
	const newPdfDoc = await fillPlacementAuth(req.body);
	const documentTitle = `${req.body.aNumber}_Placement_Authorization.pdf`;
...
async function fillPlacementAuth(child) {
	const pdfData = fs.readFileSync(
		path.join(__dirname, '../placement-auth-template.pdf'),
		{
			encoding: 'base64',
			flag: 'r',
		}
